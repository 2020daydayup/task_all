...
class MemoAdmin():
        ...
        while keep:
            ...
            try:
                ...
                if   select == 1: self.add(username)
                    ...
                    a = input("请选择返回格式，4：按年分组返回，7按月分组返回，其他")
                    if int(a) not in (4, 7):
                        keep = 1
                    else:
                        self.query_json(username=username,model=int(a))
            except:
                print("数据不符合需求")
                keep = 1
    ...
    def query_json(self,username, model=7):
        "所有记录分组后返回json，"
        memo_dict = {}
        all_memo = self.query(model=2, username=username)[1]
        all_memo.sort(key=itemgetter('date'))
        for key, group in groupby(all_memo, key=lambda m: m['date'][:model]):
            memo_dict[key] = list(group)
        memo_json = json.dumps(memo_dict, indent=4)
        self.memo_send_mail(data=memo_json, username=username, tosb="longxj@geneplus.org.cn")
        return memo_json
    ...
    def memo_send_mail(self, data, username, tosb):
        "发送邮件"
        #从配置文件读取邮件参数
        config = configparser.ConfigParser()
        config.read("config/mail.config")
        item = dict(config.items('mail'))
        s = smtplib.SMTP(item['host'])
...
class Register():
    ...
    def verify(self):
        ...
        if username in user_dict.keys():
            ...
            with open(r'config/config.properties', 'a+') as f_c:
                config.write(f_c)
